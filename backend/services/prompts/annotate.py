...
OVERVIEW_USER_TEMPLATE = """根据以下 Python 项目的调用图骨架（由 AST 静态分析提取），将运行流程归纳为标准流程图。
...
7. id 使用字符串数字 "1", "2", "3" ...
8. process 节点必须提供 file 和 symbol 字段：
   - file: 该步骤所在的文件路径（直接使用骨架中给出的路径）
   - symbol: 该步骤对应的函数名或类名（直接使用骨架中给出的名称）
   注意：不要返回 line 或 code_snippet 字段，行号会由系统自动填充
...
def _format_skeleton(graph_dict: dict) -> str:
    ...
    if entry_defs:
        lines.append("## 入口点（API endpoints / 启动函数）")
        for qname, d in entry_defs.items():
            decorators = d.get("decorators", [])
            params = ", ".join(d.get("params", []))
            doc = d.get("docstring") or ""
            dec_str = " ".join(decorators) + " " if decorators else ""
            sig = f"  {dec_str}{d['kind']} {d['name']}({params})  [{d['file']}]"
            if doc:
                sig += f'  # "{doc}"'
            lines.append(sig)
...
def build_overview_prompt(graph_dict: dict) -> tuple[str, str]:
    """Build the semantic overview flowchart prompt from AST skeleton.

    Uses the same output format as the old build_flowchart_prompt,
    but input is the compact AST skeleton (signatures + call relationships)
    instead of full source code. This dramatically reduces token usage.
    """
    skeleton = _format_skeleton(graph_dict)
